...
describe("createImageClient", () => {
  it("forces the mock client during dry-run", () => {
    expect(createImageClient(config())).toBeInstanceOf(MockImageClient);
  });

  it("creates OpenAI only for an explicit image check", () => {
    expect(
      createImageClient(config(), { forceOpenAi: true }),
    ).toBeInstanceOf(OpenAiImageClient);
  });
});
